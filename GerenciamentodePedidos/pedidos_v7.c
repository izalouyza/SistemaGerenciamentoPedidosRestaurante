...
void exibir_card() {
    if(codigo > 0) {
        ...
        for(int x = 0; x < 5; x++){
            tab[x] = (char *)malloc(20*sizeof(char));
            strcpy(tab[x], strings[x]);
        }
...
void criar_pedido_menu() {
    if (codigo > 0){ // Se tiver itens cadastrados
        ...
        int *num_itens_pedido = (int *)malloc(sizeof(int));
...
void alterar_status() {
    if(num_pedidos > 0){
        ...
        //int cod_pedido;
        int *cod_pedido = (int *)malloc(sizeof(int));
        while(1){
    ...
    if(num_pedidos > 0){
        ...
        if (*cod_pedido > 0 && *cod_pedido <= num_pedidos) {
            ...
            if (*num_itens_pedido > 0 && *num_itens_pedido <= max){
                ...
                for (int i = 0; i < *num_itens_pedido; i++) {
                    //int codigo_item;
                    int *codigo_item = (int *)malloc(sizeof(int));
                    while(1){
                        printf("Código do item %d do pedido: ", i + 1);
                        if(scanf("%d", codigo_item) != 1){
                            printf("Valor inválido. Por favor, insira um número válido.\n");
